...
maxDPDT = B(:,7);
minDPDT = B(:,8);
 [h1, p1] = ttest2(heartbeat(1:6) , distolicP(7:13));
 disp('Heart rate (hval) ', num2str(h1), 'pval ',num2str(h1));
  [h2, p2] = ttest2(diastolicP(1:6) , heartbeat(7:13));
 disp('Diastp(hval) ', num2str(h2), 'pval ',num2str(h2));
 [h3, p3] = ttest2(systolicP(1:6) , systolicP(7:13));
 disp('Systp(h val) ', num2str(h3), ' pval ',num2str(h3));
  [h4, p4] = ttest2(systolicP(1:6) , systolicP(7:13));
 disp('Systp(h val) ', num2str(h3), ' pval ',num2str(h3));
